...
또한 특정 기업이나 서비스에 편향되지 않도록 일반적인 AI 기술 용어(AI, 인공지능, 생성형 AI, LLM 등)와 주요 AI 서비스를 함께 포함하여 키워드를 구성했습니다. 이를 통해 다양한 언론사의 AI 관련 기사를 폭넓게 수집하고, 특정 키워드에 치우친 필터링이 발생하지 않도록 관리합니다.
...
2. 뉴스의 내용의 감성을 긍정, 중립, 부정 중 하나로 분류하세요.
...
| Gemini AI 호출 오류 | Error Handler의 Retry 기능을 통해 최대 2회 자동 재시도 후에도 실패하면 Make 실행 이력(History)에서 확인 | 일시적인 API 오류에 자동 대응하고 안정성을 높이기 위해 |
...
전체 기능 구현 후 실행 테스트를 진행한 결과, 뉴스 수집, 키워드 필터링, 중복 확인, AI 요약 및 감성 분석, 노션 저장, Discord 알림 전송이 모두 정상적으로 동작하는 것을 확인했습니다. 이를 통해 데이터 수집부터 AI 가공, 저장, 알림까지 사람의 개입 없이 자동으로 수행되는 뉴스 요약 시스템을 완성했으며, Make를 활용한 업무 자동화와 AI 서비스 연계 방식을 직접 구현하고 활용하는 경험을 얻을 수 있었습니다.
